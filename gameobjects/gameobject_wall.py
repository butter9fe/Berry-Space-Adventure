...
class Wall(GameObject_Base):
    def __init__(self, canvas: tk.Canvas, is_left: bool, list_images: list[tk.PhotoImage]):
        ...
        super().__init__(GameObjectType.WALL, canvas, position, Vector2(25, canvas.winfo_height()))

    def draw(self) -> int:
        return self.canvas.create_line(self.position.x, 0, self.position.x, self.canvas.winfo_height(), fill='white', width=25)
